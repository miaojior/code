...
double scores[Students][Subjects],averages,excellent_rates[Subjects];
...
int main(){
    ...
    for(int i=0;i<Students;i++){
        ...
        for (int j = 0; j < Subjects; j++) {
            std::cout<<"请输入"<<(j==0?"高等数学":(j==1?"英语":"物理"))<<"的成绩：";
            std::cin>>scores[i][j];
        }
    }
    ...
    for(int i=0;i<Students;i++){
        ...
        for(int j=0;j<Subjects;j++){
            std::cout<<(j==0?"高等数学":(j==1?"英语":"物理"))<<": "<<scores[i][j]<<std::endl;
            sum+=scores[i][j];
        }
    ...
    for (int j=0;j<Subjects;j++){
        ...
        for (int i=0;i<Students;i++)
            if (scores[i][j] >= 90)
                excellent_count++;
        excellent_rates[j]=(double)excellent_count/Students*100;
        std::cout<<(j==0?"高等数学":(j==1?"英语":"物理"))<<"课优秀率：";
        printf("%.2lf\n",excellent_rates[j]);
    }
    return 0;
}